...
const AccessoryProductPage = () => {
  ...
  const selectedAccessoryProductMem = useSelectedAccessoryProductStore(
    (state) => state.selected_accessory_product
  );
  ...
  const isProductInCart = CartItems.some(
    (item) =>
      item.accessory_id === selectedProduct?.accessory_id &&
  ...
  const parsedDescription = parseProductDescription(
    selectedProduct?.accessory_description
  );
  ...
  useEffect(() => {
    if (selectedAccessoryProductMem?.accessory_id) {
      setSelectedProduct(selectedAccessoryProductMem);

      // insert as recently viewd
      addUniqueUserViewedProduct(selectedAccessoryProductMem);
    }
  }, [selectedAccessoryProductMem?.accessory_id]);
  ...
  const productMedia = selectedProduct?.accessory_gallery || [];
  const productSizes = selectedProduct.accessory_sizes || [];
  ...
  const handleAddToCart = () => {
    ...
    // build a new accessory product object with selected size
    const productToAdd = {
      ...selectedProduct,
      selected_size: {
  ...
  const handleRemoveFromCart = () => {
    ...
    console.log("deleting accessory product from cart");
    ...
    if (mediaItem.type === "video") {
      ...
      return (
        ...
        >
          <Image
            src={mediaItem.url}
            alt={`${selectedProduct.accessory_name} image ${index + 1}`}
  ...
  if (!selectedProduct?.accessory_id) {
    return (
      <div>
        <Navbar />
        <div className="flex justify-center items-center h-screen">
          <p>No accessory product selected</p>
        </div>
        <Footer />
      </div>
    );
          ...
          <div className="max-w-2xl mx-auto w-full">
            ...
            <div className="border-b border-gray-200 pb-6">
              <h1 className="text-2xl lg:text-3xl font-light tracking-wide text-gray-900">
                {selectedProduct.accessory_name}
              </h1>
