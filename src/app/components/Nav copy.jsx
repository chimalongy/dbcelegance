...
export default function Navbar() {
  ...
  return (
    <nav className={`${showrightnavoptions ? "bg-black/20 " : ""} absolute w-full px-4 py-3 md:px-6 md:py-4 flex justify-between items-center z-50`}> 
      {/* Left Section - Hamburger & Search */}
      <div className='flex items-center'>
        <button
          onClick={() => setMenuOpen(!menuOpen)}
          className="p-2 focus:outline-none focus:ring-2 focus:ring-white/30 rounded-md transition-all duration-200 hover:bg-white/10"
          aria-label="Toggle menu"
        >
          {menuOpen ? (
            <IoMdClose className="text-xl text-white" />
          ) : (
            <HiOutlineEquals className="text-xl text-white" />
          )}
        </button>

        <button
          className="text-white hover:text-white/80 transition-colors p-2 ml-2 rounded-md hover:bg-white/10"
