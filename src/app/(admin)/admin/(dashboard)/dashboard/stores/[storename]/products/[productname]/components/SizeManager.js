...
export default function SizeManager({
  ...
  useEffect(() => {
    if (!skuEdited && formData.size) {
      const autoSku = `${slugify(productName)}-${slugify(formData.size)}`;
      setFormData((prev) => ({ ...prev, sku: autoSku }));
    }
  }, [formData.size, productName, skuEdited]);

  // Auto SKU for add modal
  useEffect(() => {
    if (!newSkuEdited && newSizeData.size) {
      const autoSku = `${slugify(productName)}-${slugify(newSizeData.size)}`;
      setNewSizeData((prev) => ({ ...prev, sku: autoSku }));
    }
  }, [newSizeData.size, productName, newSkuEdited]);
